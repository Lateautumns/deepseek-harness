  ...
  vi.doMock('koffi', () => ({
    default: {
      ...
      decode: (value: unknown, offsetOrType: unknown, lengthOrType?: unknown): unknown => {
        if (offsetOrType === 'char16_t' && lengthOrType === -1) return (value as FakePtr).text
...
describe('loadWin32DialogBindings over the fake COM world', () => {
  ...
  it('keeps UTF-16 code units whose low byte is zero in the selected path', async () => {
    const path = 'D:\\02_开发项目\\PLM\\RdcPlatform'
